...
/**
 * Created by Sergey Karleev on 07.06.2019.
 */
/*
...
class CoroutineCallAdapterFactory private constructor() : CallAdapter.Factory() {
    ...
    private class ResponseCallAdapter<T>(
        ...
        override fun adapt(call: Call<T>): Deferred<Response<T>> {
            ...
            call.enqueue(object : Callback<T> {
                override fun onFailure(call: Call<T>, t: Throwable) {
                    deferred.completeExceptionally(Throwable(t))
                }

                override fun onResponse(call: Call<T>, response: Response<T>) {
                    val url: String = call.request().url.toUrl().toString()
                    if (response.isSuccessful) {
                        deferred.complete(response)
                    } else {
                        val errorMessage = response.errorBody()?.string() ?: "Something went wrong"
                        val errorCode = response.code()
                        val causeThrowable = Throwable("$errorCode")
                        deferred.completeExceptionally(Throwable(errorMessage, causeThrowable))
                    }
                }
            })
